...
import { TEST_IDS, createAnonClient, createServiceRoleClient } from './utils'
...
describe('Database Setup Integration', () => {
  ...
  describe('Connection', () => {
    it('should connect to test Supabase with service role', async () => {
      const { data, error } = await serviceClient.from('raffles').select('count').limit(1)
    ...
    it('should connect to test Supabase with anon key', async () => {
      const { error } = await anonClient.from('raffles').select('count').limit(1)
  ...
  describe('Seed Data', () => {
    it('should have test raffles seeded', async () => {
      const { data, error } = await serviceClient.from('raffles').select('*').order('created_at')
  ...
  describe('RLS Policies', () => {
    it('should allow anon to read active raffles', async () => {
      const { data, error } = await anonClient.from('raffles').select('*').eq('status', 'active')
    ...
    it('should prevent anon from reading draft raffles', async () => {
      const { data, error } = await anonClient.from('raffles').select('*').eq('status', 'draft')
